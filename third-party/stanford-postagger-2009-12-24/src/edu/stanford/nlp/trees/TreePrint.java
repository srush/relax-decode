    ...
    if (propertyToBoolean(options, "xml")) {
      ...
      if (formats.containsKey("conll2008")) {
        ...
        for (int wi=0; wi<tagged.size(); ++wi) {
          ...
          int depi = dep.<Integer, CoreAnnotations.IndexAnnotation>get(CoreAnnotations.IndexAnnotation.class);
          int govi = gov.<Integer, CoreAnnotations.IndexAnnotation>get(CoreAnnotations.IndexAnnotation.class);
      ...
      if (formats.containsKey("conllStyleDependencies")) {
        ...
        if (failed) {
          ...
          for (Dependency<Label, Label, Object> dep : depsSet) {
            ...
            int childIndex = child.<Integer, CoreAnnotations.IndexAnnotation>get(CoreAnnotations.IndexAnnotation.class);
            int parentIndex = parent.<Integer, CoreAnnotations.IndexAnnotation>get(CoreAnnotations.IndexAnnotation.class);
//            System.err.println(childIndex+"\t"+parentIndex);
  ...
  private static String toXMLString(Collection<TypedDependency> dependencies) {
    ...
    for (TypedDependency td : dependencies) {
      ...
      String govCopy = "";
      if (td.gov().label.get(CopyAnnotation.class) != null && td.gov().label.get(CopyAnnotation.class) != null) {
        govCopy = " copy=\"yes\"";
      }
      String depCopy = "";
      if (td.dep().label.get(CopyAnnotation.class) != null && td.dep().label.get(CopyAnnotation.class) != null) {
        depCopy = " copy=\"yes\"";
      }
