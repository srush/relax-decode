...
bool SplitHeuristic::has_value(const Location & l,
...
double SplitHeuristic::get_value(const Location & l,
                                 const Hypothesis & hyp) const {
  if (l.location == NODE) {
    int node_id = l.node_id;
    return
        _outside_scores.store[node_id]->get_score_by_id(lower_id(hyp));
  } else if (l.location == EDGE) {
    int edge_id = l.edge_id;
    return
        (*_outside_edge_scores.store[edge_id])
        [l.edge_pos].get_score_by_id(lower_id(hyp));
  }
  assert(false);
  return 0.0;
}

SplitController::SplitController(const Subproblem &s,
...
void SplitController::initialize_hypotheses(const Hypernode & node,
  ...
  if (_subproblem.is_overridden(graph_id)) {
    ...
    for (int d = 0; d < dim(); d++) {
      ...
      for (int d2 = 0; d2 < dim(); d2++) {
        ...
        right_side[0] = project_word(graph_id);
        right_side[1] = d;
        Hypothesis * h = new Hypothesis(State(hooks, dim()),
                                        State(right_side, dim()));
...
void SplitController::initialize_out_root(vector <Hypothesis *> & hyps,
  ...
  double SplitController::find_best(vector <Hypothesis *> & root_hyps,
