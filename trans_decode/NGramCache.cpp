...
static const bool lm_dummy =
    RegisterFlagValidator(&FLAGS_lm_file, &ValidateFile);

DEFINE_double(lm_weight, 0.0, "lm weight value");
static const bool lm_weight_dummy =
    RegisterFlagValidator(&FLAGS_lm_weight, &ValidateNum);


// assume we are at a point in the trie, just look one step more
LogP NgramCache::wordProbFromCache(VocabIndex word,
...
LogP NgramCache::wordProbPrimeCache(VocabIndex word,
                                    const VocabIndex *context) {
