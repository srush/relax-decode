...
void Decode::add_subgrad(wvector *subgrad,
  ...
  if (!first) {
    ...
    if (DEBUG) {
      assert(fabs(-b1_lag -
                  _subproblem->get_best_bigram_weight(start_from,
                                                      mid_at, 0)) < 1e-3);
  ...
  if (DEBUG) {
    assert((-b2_lag -
            _subproblem->get_best_bigram_weight(mid_at, end_at, 1)) < 1e-3);
    lag_total +=  b2_lag;
    local_lag_total +=  b2_lag;
  }

  if (DEBUG) {
    double lm_score = (lm_weight()) *
        _subproblem->word_prob_reverse(start_from, mid_at, end_at);
    double w =0;
...
void Decode::print_output(const wvector & subgrad) {
  ...
  for (wvector::const_iterator it = subgrad.begin();
       it != subgrad.end(); it++) {
    if (it->second != 0.0) {
      cout << it->first << " "
           << it->second << " " << (*_lagrange_weights)[it->first]<< endl;
      if (it->first < GRAMSPLIT && _lattice.is_word(it->first)) {
        cout << _lattice.get_word(it->first)
             << " " << _subproblem->project_word(it->first) <<  endl;
        bis++;
      } else if (it->first < GRAMSPLIT && _lattice.is_word(it->first)) {
        cout << _lattice._edge_label_by_nodes[it->first] <<  endl;
      }
      if (it->first >= GRAMSPLIT && it->first < GRAMSPLIT2
          && _lattice.is_word(it->first - GRAMSPLIT)) {
        cout << _lattice.get_word(it->first - GRAMSPLIT)
             << " " << _subproblem->project_word(it->first -GRAMSPLIT) << endl;
        tris++;
      } else if (it->first >= GRAMSPLIT && it->first < GRAMSPLIT2) {
        cout << _lattice._edge_label_by_nodes[it->first- GRAMSPLIT] <<  endl;
      }
      if (it->first >= GRAMSPLIT2 &&
          _lattice.is_word(it->first - GRAMSPLIT2)) {
        cout << _lattice.get_word(it->first - GRAMSPLIT2)
             << " " << _subproblem->project_word(it->first -GRAMSPLIT2) << endl;
        tris++;
...
double Decode::best_modified_derivation(const EdgeCache & edge_weights,
  ...
  if (run_astar) {
    ...
    if (TIMING) {
      ...
      cout << "OUTSIDE time: " << Clock::diffclock(end, begin) << " ms"<< endl;
      begin = clock();
    }
...
void Decode::remove_lm(int feat, wvector & subgrad,
...
wvector Decode::construct_lm_subgrad(
  ...
  // FULLBUILT: The graph will have <s_1> on twice, but the lat we
  // only use it once in gramsplit, so fake it
  remove_lm(used_lats[0], subgrad, dual, cost_total);

  // FULLBUILT: The graph will have <e_1> on twice, but we use it only
  // once
  remove_lm(used_lats[used_lats.size()-2] + GRAMSPLIT,
            subgrad,
            dual,
            cost_total);
  ...
  remove_lm(used_lats[used_lats.size()-1],
            subgrad,
            dual,
            cost_total);
  remove_lm(used_lats[used_lats.size()-1] + GRAMSPLIT,
            subgrad,
            dual,
            cost_total);
  return subgrad;
...
void Decode::solve(const SubgradState & cur_state,
  ...
  vector<const ForestNode *> used_words;
  {
    vector<const Hypernode *> tmp_words =
        ha.construct_best_fringe(back_pointers);
    foreach (const Hypernode * word, tmp_words ) {
      used_words.push_back(static_cast<const ForestNode *>(word));
    }
  }
  ...
  result.subgrad +=
      construct_lm_subgrad(used_words,
                           used_lats,
                           used_strings,
                           result.dual,
                           cost_total);
  ...
  if (TIMING) {
    ...
    cout << "CONSTRUCT LAGRANGIAN TIME: "
         << Clock::diffclock(end, begin) << " ms"<< endl;
    begin = clock();

    end = clock();
    cout << "COMPUTE PRIMAL TIME: "
         << Clock::diffclock(end, begin) << " ms"<< endl;

    cout << "TOTAL TIME: "
         << Clock::diffclock(end, total_begin) << " ms"<< endl;
  }
...
double Decode::compute_primal(const HEdges used_edges,
  ...
  for (uint i =0; i < used_strings.size()-2; i++) {
    ...
    if (DEBUG) {
      double lm_score = (lm_weight()) *
          _lm.wordProb(lookup_string(used_strings[i+2]), context);
      cout << "PRIMAL " << used_strings[i] << " " <<  used_strings[i+1]
           << " " <<  used_strings[i+2] << " " << lm_score << endl;

      int start_from =
          _lattice.get_word_from_hypergraph_node(used_nodes[i+2]->id());
      int mid =
          _lattice.get_word_from_hypergraph_node(used_nodes[i+1]->id());
      int end =
          _lattice.get_word_from_hypergraph_node(used_nodes[i]->id());
      double w =0;
...
void Decode::debug(int start_from,
                   int dual_mid,
                   int dual_end,
                   int primal_mid,
                   int primal_end) {
  if (SIMPLE_DEBUG) {
    ...
    cout << setw(3) << primal_end << " "
         << setw(2) << _subproblem->project_word(primal_end) << " "
         << setw(15) << _lattice.get_word(primal_end) <<  " "
         << setw(3) << primal_mid << " "
         << setw(2) << _subproblem->project_word(primal_mid) << " "
         << setw(15) << _lattice.get_word(primal_mid);
    cout << ("  "+diff+"   ")
         << setw(3) << dual_end << " "
         << setw(2) << _subproblem->project_word(dual_end) << " "
         << setw(15)<<  _lattice.get_word(dual_end) << " "
         << setw(3) << dual_mid <<" "
         << setw(2)<< _subproblem->project_word(dual_mid) << " "
         << setw(15) <<_lattice.get_word(dual_mid) << " " << over << " "
         << setw(15)<< _lattice.get_word(start_from) << endl;
  }
}



void Decode::greedy_projection(int dual_mid,
                               int dual_end,
                               int primal_mid,
                               int primal_end) {
