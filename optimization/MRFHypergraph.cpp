...
MRFHypergraph MRFHypergraph::from_mrf(const MRF & mrf) {
  ...
  foreach (Node n, nodes) {
    ...
    foreach (Edge e, n->edges()) {
      ...
      Cache <State, Hypernode * > * c2 = new  Cache <State, Hypernode * >(mrf.states(*to_node).size());
    ...
    foreach (const State & my_s, mrf.states(*n) ) {
      ...
      mrf_hyp._canonical_hnode->set_value(mrf.make_assignment(*n, my_s), base_hnode);
      ...
      foreach (Edge e, n->edges()) {
        ...
        foreach (const State & to_s, mrf.states(*to_node)) {
          ...
          stringstream wstr;
          wstr << "value="<< -mrf.edge_pot(*e, my_s, to_s);
  ...
  foreach (const State & last_state, mrf.states(*root) ) {
    ...
    HyperedgeImpl * edge = new HyperedgeImpl("", new wvector(), hedge_id, tail_nodes, mrf_hyp._root);
    hedge_id++;
    mrf_hyp._edges.push_back(edge);
